...
![图 1 直观对比了对话记忆与空间记忆在结构上的根本差异：前者是扁平的线性文本，主要服务语言交互；后者是时空锚定的图结构，跨会话持久存在，支持证据追溯与不确定度建模。](../diagrams/article/01-chat-memory-vs-world-memory.png)
...
![图 2 以时间线的形式呈现了这一百年演化的四个阶段：从行为学假说到神经科学发现，再到计算理论与工程系统，每个阶段都在为"如何把连续时空经历组织为可查询的内部表征"这同一个核心问题提供新的回答。](../diagrams/article/02-cognitive-map-to-engineering-timeline.png)
...
![图 3 以"能力断层"的可视化方式直观呈现了这一节的核心论点：感知层的能力高原与认知/记忆层的能力谷地之间存在一道结构性悬崖，且三股外部压力——具身 AI、自动驾驶、XR——正同时把系统需求推向悬崖的另一侧。](../diagrams/article/03-perception-cognition-cliff.png)
...
![图 4 用一张总览图把全文最重要的判断压缩成了三句话：感知负责把外部信号转成观测，认知负责把观测压缩成可操作的世界状态，记忆负责把这个状态跨时间稳定下来，并保留证据与置信度。](../diagrams/article/04-three-layer-architecture-overview.png)
...
![图 5 从可解释性、在线更新、不确定性建模、跨时间查询和设备能效五个维度，对比了全隐式端到端路线与神经符号混合路线在满足空间记忆核心需求上的结构性差异。](../diagrams/article/05-end-to-end-vs-neurosymbolic.png)
...
![图 6 这张图想说明的是：现有基准并非一无是处，但它们主要在测“看见了什么”，还没有真正测“系统能否跨时间记住并维护一个世界状态”。](../diagrams/article/06-benchmark-blind-spot.png)
...
![图 7 以可视化的方式呈现了这个类比：左侧"洞穴之内"是当今 AI 系统的现状——精于识别影子（单帧识别、图像描述、视觉问答），却缺少稳定的世界状态；右侧"洞穴之外"是空间记忆所承诺的方向——持久的世界状态、校准的不确定性、可追溯的证据链。](../diagrams/article/07-cave-allegory-spatial-memory.png)
