...
1.  创建程序版本号（通过Version类）


2.  保证只有一个WorldWind实例在运行
3.  读取初始化配置文件（C:\Documents and Settings\\user\>\ApplicationData\NASA\WorldWind\1.3.5.0\WorldWind.xml），读取的方法是利用XML反序列化实现的
4.  添加线程异常事件处理函数Application_ThreadException
5.  创建MainApplication实例
5.1. 判断是否需要运行配置向导    5.2. 创建启动画面（Splash类，是一个Form的派生类）

    5.3. 配置缓存    5.4. 设置配置文件路径并读取配置文件    5.5. 将读出的配置信息写入菜单项    5.6. OpenStartupWorld

    5.6.1. 确定应该打开哪个星球的视图    5.6.2. OpenWorld("星球配置文件");
